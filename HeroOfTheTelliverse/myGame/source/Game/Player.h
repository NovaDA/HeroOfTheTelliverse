...
namespace Rendering
{
	...
	public:
		Player(Game& game, Camera& camera, const std::string modelFilename);
		Player(Game& game, Camera& camera, const std::string modelFilename, const std::wstring modelDes, int modelValue);
		~Player();
		...
		Keyboard* mKeyboard;
		XMFLOAT3 getPosition(); //returns the position of the object
	private:
		...
		XMFLOAT3 positionModel;											/// VIP  THIS IS THE VARIABLE THAT PASS THE MODEL POSITION TO CAMERA
		float mMovementRate;
